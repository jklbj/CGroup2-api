...
MEM_INFO = YAML.load_file("#{DIR}/groups_members.yml")
...
def add_members
  ...
  member_info.each do |member|
    group = CGroup2::Group.first(name: member['group_title'])
    account = group.owner
    member['member_email'].each do |email|
      invitee = CGroup2::Account.first(email: email)
      CGroup2::AddMember.call(
        group, account ,invitee
      )
    end
  end
end
